...
void Graphics::debug_thread_team1(){
  ...
  while(ros::ok()){
    ...
    for(int i = 0; i < global_debug_team1.step_poses.size(); i++) {
      // REVIEW antes estava y nos dois o que nao faz sentido, comparar com codigo antigo
      if(global_debug_team1.step_poses[i].y && global_debug_team1.step_poses[i].x) {
    ...
    for(int i = 0; i < global_debug_team1.final_poses.size(); i++) {
      // REVIEW antes estava y nos dois o que nao faz sentido, comparar com codigo antigo
      if(global_debug_team1.final_poses[i].y && global_debug_team1.final_poses[i].x) {
    ...
    //! Recebe os caminhos
    for (int i = 0; i < global_debug_team1.paths.size(); i++) {
      Path path;
      for (int j = 0; j < global_debug_team1.paths[i].poses.size(); j++) {
        Pose pose;
...
void Graphics::debug_thread_team2() {
  ...
  while(ros::ok()){
    ...
    //! Recebe os vetores de movimento
    for(int i = 0; i < global_debug_team2.step_poses.size(); i++) {
      if(global_debug_team2.step_poses[i].y && global_debug_team2.step_poses[i].x) {
    ...
    //! Recebe as poses finais
    for(int i = 0; i < global_debug_team2.final_poses.size(); i++) {
      if(global_debug_team2.final_poses[i].x != 0 && global_debug_team2.final_poses[i].y != 0){
    ...
    //! Recebe os caminhos
    for(int i = 0; i < global_debug_team2.paths.size(); i++) {
      Path path;
      for(int j = 0; j < global_debug_team2.paths[i].poses.size(); j++) {
        Pose pose;
...
void Graphics::state_thread(){
  ...
  while(ros::ok()){
    ...
    for(int i = 0 ; i < 3 ; i++){
      ...
      // REVIEW conferir como ele lida com o fato do simulator nao enviar cor

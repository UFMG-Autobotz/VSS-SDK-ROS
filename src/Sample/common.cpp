...
namespace common{
    ...
    State global_state2State(vss_sdk_ros::global_state global_state, string main_color){
        State state;

        state.ball.x = global_state.ball.pose.x;           // Pos X
        state.ball.y = global_state.ball.pose.y;           // Pos Y

        state.v_ball.x = global_state.ball.v_pose.x;         // Vel X
        state.v_ball.y = global_state.ball.v_pose.y;         // Vel Y

        state.ball_kalman.x = global_state.ball.k_pose.x;         // Kalman Pos X
        state.ball_kalman.y = global_state.ball.k_pose.y;         // kalman Pos Y

        state.v_ball_kalman.x = global_state.ball.k_v_pose.x;       // Kalman Vel X
        state.v_ball_kalman.y = global_state.ball.k_v_pose.y;       // Kalman Vel Y
